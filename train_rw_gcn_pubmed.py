...
with torch.no_grad():
    ...
    for subgraph in content_g:
        ...
        #        test_acc_score[idx_test_dict[i]].append(output[i])
...
#     acc_sum = 0
#     num_null_test = 0
#     for i in test_acc_score:
#         len_test = len(test_acc_score[i])
#         if(len_test== 0):
#             num_null_test += 1
#             continue
#         else:
#             num_test += 1
#             test_i = np.sum(test_acc_score[i], axis=0)/len_test
...
#print('# null and yes test nodes:', num_null_test, num_yes_test)
#print('\n')
print("Test set results:",
          "loss= {:.4f}".format(loss_sum/num_test),
          "acc= {:.4f}".format(acc_sum/num_test))
...
f = open('./results/pubmed_gcn_ripplewalk_size3000_num50', 'wb')
pickle.dump(data, f)
f.close()
